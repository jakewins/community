        ...
        org.neo4j.unsafe.batchinsert.BatchInserterIndex
{
    ...
    LuceneBatchInserterIndex( BatchInserter inserter,
            IndexIdentifier identifier, Map<String, String> config )
    {
        if ( inserter == null )
            new Exception( "blaaa" ).printStackTrace();
        String indexDir = new File( new File( inserter.getStoreDir(), "index" ), "lucene" ).getAbsolutePath(); //AbstractIndexImplementation.getIndexStoreDir( dbStoreDir, LuceneDataSource.DATA_SOURCE_NAME );
        ...
        this.writer = instantiateWriter( indexDir );
    }
    ...
    public void add( long entityId, Map<String, Object> properties )
    {
        try
        {
            EntityId idObject = getEntityId( entityId );
            Document document = IndexType.newDocument( idObject );
    ...
    private EntityId getEntityId( long id )
    {
        if ( identifier.getEntityType() == EntityType.NODE )
            return EntityId.entityId( id );
        BatchRelationship rel = inserter.getRelationshipById( id );
        return EntityId.entityId( id, rel.getStartNode(), rel.getEndNode() );
    }
    ...
    private IndexHits<Long> query( Query query, final String key, final Object value )
    {
        try
        {
            ...
            IndexHits<EntityId> iterator = null;
            if ( key == null || this.cache == null || !this.cache.containsKey( key ) )
            {
                iterator = new DocToIdIterator( result, Collections.<EntityId>emptyList(), null );
            }
            else
            {
                iterator = new DocToIdIterator( result, Collections.<EntityId>emptyList(), null )
                {
                    private final Collection<EntityId> ids = new ArrayList<EntityId>();
                    
                    @Override
                    protected EntityId fetchNextOrNull()
                    {
                        EntityId result = super.fetchNextOrNull();
                        if ( result != null )
                        {
                            ids.add( result );
                        }
...
//                        addToCache( ids, key, value );
            ...
            final IndexHits<EntityId> idIterator = iterator;
            return new AbstractIndexHits<Long>()
            {
                @Override
                public int size()
                {
                    return idIterator.size();
                }

                @Override
                public float currentScore()
                {
                    return idIterator.currentScore();
                }

                @Override
                protected Long fetchNextOrNull()
                {
                    return idIterator.hasNext() ? idIterator.next().getId() : null;
                }
            };
        }
        catch ( IOException e )
